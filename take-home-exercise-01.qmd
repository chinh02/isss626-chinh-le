...
There are five questions I want to explore:
...
| [OpenStreetMap roads](https://www.openstreetmap.org/copyright), retrieved through [Overpass](https://overpass-api.de/) | A closer look at main roads and connectors in eastern Bangkok, as mapped on 1 January 2022. |
...
The closer look at eastern Bangkok points to Routes 7 and 9, which account for about 86% of its matched reports. The local road-distance test gives no clear evidence of extra clustering in time here. I would keep that distinction in mind when deciding which places need further investigation.
...
- Review patrol and emergency-response coverage in Bangkok and Samut Sakhon, where reports rose late in 2022. Use recent accident and dispatch records to confirm when extra coverage is needed before changing schedules.
...
- Use crash severity and traffic volumes alongside these maps to rank road improvements. Confirm the location and conditions on site before choosing a treatment, so spending follows the problems found at each road section.
...
- [OpenStreetMap contributors](https://www.openstreetmap.org/copyright), roads as mapped on 1 January 2022, retrieved through [Overpass](https://overpass-api.de/). Open Database License 1.0.
